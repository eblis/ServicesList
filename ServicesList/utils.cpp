...
int IsPluginLoaded(char *pluginName)
{
	HANDLE hFind;
	...
	PLUGININFO* (*MirandaPluginInfo)(DWORD); //These two are used to get informations from the plugin.
	PLUGININFO *pluginInfo = NULL; //Read above.
	...
	if(hFind != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			if (hInstPlugin) //is the plugin loaded ?
			{
				MirandaPluginInfo = (PLUGININFO* (*)(DWORD))GetProcAddress(hInstPlugin, "MirandaPluginInfo");
				if (MirandaPluginInfo)
				{
					pluginInfo = MirandaPluginInfo(mirandaVersion);
					if (_stricmp(pluginInfo->shortName, pluginName) == 0)
					{
						found = TRUE;
					}
				}
